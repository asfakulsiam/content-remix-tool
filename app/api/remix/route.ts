  ...
  try {
    ...
    if (!process.env.OPENROUTERAI_API_KEY) {
      return NextResponse.json(
        { error: "OpenRouter API key is missing. Please add OPENROUTERAI_API_KEY to your environment variables." },
        { status: 500 },
      )
    }
    ...
    const prompt = promptTemplates[format as keyof typeof promptTemplates] || promptTemplates.tweet
    ...
    return NextResponse.json({ error: "Failed to remix content" }, { status: 500 })
  }
}
